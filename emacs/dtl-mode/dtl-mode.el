...
(defconst dtl-keywords
  ...
  ;; '("UNPAIRED" "LEFTSIDE" "RIGHTSIDE" "SORTEDOUTPUT" "STLF" "CRLF" "STREAM" "OVERWRITE" "FILE" "CREATE" "LAYOUT" "CHARACTER" "SORT" "NODUPLICATE" "DELIMITED" "EXPORT" "VALUES" "FIELDSEPARATOR" "ENCLOSEDBY" "ALIAS" "RETAINIF" "DATETIME" "CHARACTER" "LOB" "FILLER" "ENCODING" "NOTNULLABLE" "NULLIFEMPTY" "EN" "SN" "REPEAT"))
...
(defun dtl-run ()
	...
	(save-excursion
		(let* ((fname (buffer-file-name))
			...
			(cd fdir)
			(shell-command (format ". job-envvars.sh dmexpress /run %s" fdir (buffer-file-name)))
			)))
